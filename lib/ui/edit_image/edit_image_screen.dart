...
class EditImageScreen extends StatelessWidget {
  ...
  const EditImageScreen({required this.file, required this.fileStat, Key? key}) : super(key: key);

  @override
  Widget build(BuildContext context) {
    return EditImageLayout(file: file,fileStat: fileStat,);
  }
}

class EditImageLayout extends StatefulWidget {
  final File file;
  final FileStat fileStat;
  const EditImageLayout({required this.file, required this.fileStat, Key? key}) : super(key: key);

  @override
  _EditImageLayoutState createState() => _EditImageLayoutState();
}

class _EditImageLayoutState extends State<EditImageLayout> {
  ...
  Future saveImage()async{
    // await utilsSp.clearObject("LOCAL_PATH");
    File image = file;
  ...
  Future saveImageToSp(String imagePath)async{
    ...
    if(data==null){
      ...
      repositoryResponse.listImageResponse.add(ImageResponse(imagePath, blur: valueBlur, presets: valuePresets, brightness: valueBrightness));
      String jsonEncodeAllRepositoryResponse = jsonEncode(repositoryResponse);
      await utilsSp.setObject("LOCAL_PATH", jsonEncodeAllRepositoryResponse);
    }
